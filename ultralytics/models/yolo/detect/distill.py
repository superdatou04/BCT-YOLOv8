...
from ultralytics.utils.checks import check_imgsz, print_args, check_amp, check_model_file_from_stem
...
class DetectionDistiller(BaseTrainer):
    ...
    def build_optimizer(self, model, name='auto', lr=0.001, momentum=0.9, decay=1e-5, iterations=1e5, feature_model=None):
        ...
        optimizer.add_param_group({'params': g[0], 'weight_decay': decay})  # add g0 with weight_decaycriterion
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    self.loss = self.loss + (log_distill_loss + fea_distill_loss) * batch['img'].size(0) * distill_decay
                    if RANK != -1:
