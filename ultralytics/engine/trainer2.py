...
class BaseTrainer:
    ...
    def setup_teacher_model(self):
        ...
        # Ensure BN layers' weights are updated during training
    ...
    def build_optimizer(self, model, name="auto", lr=0.001, momentum=0.9, decay=1e-5, iterations=1e5):
        ...
        for module_name, module in self.teacher_model.named_modules():
            for param_name, param in module.named_parameters(recurse=False):
                fullname = f"{module_name}.{param_name}" if module_name else param_name
                if isinstance(module, bn):  # weight (no decay)
                    if param.requires_grad:
                        g[1].append(param)  # Only add BN layers that require grad            
